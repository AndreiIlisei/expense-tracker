...
// // ---- Helpers ----
// function parseDkDate(s: string | null | undefined): Date | null {
//   if (!s) return null;
//   // expected dd.mm.yyyy
...
//     mo = +m[2],
//     y = +m[3];
//   // store as UTC date (no time component)
//   return new Date(Date.UTC(y, mo - 1, d));
// }

// function parseDkAmount(s: string | null | undefined): number | null {
//   if (!s) return null;
...
//   return Math.round(n * 100); // minor units
// }
...
// async function existsDuplicateTransaction(
//   date: Date | null,
//   amountMinor: number | null,
//   rawDescription: string | null
// ) {
//   if (!date || amountMinor == null || !rawDescription) return false;
...
//     const buf = Buffer.from(await file.arrayBuffer());
//     const name = (file as any).name?.toLowerCase?.() || '';
//     const type = file.type; // may be empty in some browsers
...
//       // XLSX files are ZIPs; they start with "PK" magic bytes
...
//       // Produce objects using the first row as headers
//       records = XLSX.utils.sheet_to_json(ws, {
//         raw: false, // strings, not Date objects (we’ll parse)
//         defval: '', // keep empty cells as ""
//         // header: 1 would return arrays; default returns objects with headers
...
//     // Map Danish headers robustly
//     const H = {
//       posted: ['bogført', 'bogfort', 'bogf\u00F8rt'], // variations
//       value: ['rentedato'],
//       desc: ['beskrivelse', 'tekst'],
//       in: ['ind på konto', 'ind pa konto', 'indbetaling'],
//       out: ['ud af konto', 'udbetaling'],
//     } as const;

//     const get = (row: Record<string, string>, keys: string[] | unknown) => {
//       const foundKey = Object.keys(row).find((k) =>
//         (keys as string[]).some((h) => k.trim().toLowerCase() === h)
//       );
...
//     for (const r of records) {
//       const postedRaw = get(r, H.posted);
//       const descRaw = get(r, H.desc);
//       const inRaw = get(r, H.in);
//       const outRaw = get(r, H.out);

//       const date = parseDkDate(postedRaw);
//       const desc = normDesc(descRaw);

//       const inMinor = parseDkAmount(inRaw);
//       const outMinor = parseDkAmount(outRaw);
//       const amountMinor =
//         inMinor != null ? inMinor : outMinor != null ? -outMinor : null;
...
//       // de-dup
//       // eslint-disable-next-line no-await-in-loop
//       const dupe = await existsDuplicateTransaction(date, amountMinor, desc);
//       if (dupe) {
//         skipped++;
//         continue;
//       }
